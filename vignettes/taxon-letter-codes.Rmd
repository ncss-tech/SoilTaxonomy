...
`taxon_to_level` can also identify a fifth, lower-level tier (`level="family"`). Soil family differentia are not handled in the keys, but are defined by concatenating comma-separated class names on to the subgroup. Family classes have their own keys and apply variably depending on the subgroup-level taxonomy.

For instance, the soil family `"Fine, mixed, semiactive, mesic Ultic Haploxeralfs"` includes a particle-size class (`"fine"`), a mineralogy class (`"mixed"`), a CEC activity class (`"semiactive"`) and a temperature class (`"mesic"`)
...
Say, you have the above family level taxon and you want to determine the taxonomy at a higher (less detailed) level. 
You can readily trim off the family and subgroup-level modifiers to get the Great Group using `getTaxonAtLevel(level="greatgroup")`

```{r}
getTaxonAtLevel("Fine, mixed, semiactive, mesic Ultic Haploxeralfs", level = "greatgroup")
```

If you request a more-detailed taxonomic level, you will get an `NA` result.
...
Another handy wrapper method around taxon letter code functionality is `getParentTaxa`. This function will enumerate the tiers above a particular taxon.
...
Another function that might be useful for comparing relative positions within keys, or the number of steps that it takes to reach a particular taxon, is `relative_taxon_code_positon`.
This function counts how many taxa key out before a particular taxon along the logical path to that taxon and then adds $1$ to get the taxon position.
