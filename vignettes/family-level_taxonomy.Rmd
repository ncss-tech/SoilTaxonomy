...
```{r, include = FALSE}
.soilDB_is_sufficient <- function() (tryCatch(utils::packageVersion("soilDB"),
                                             error = function(e) "0.0.0") >= "2.7.3")
knitr::opts_chunk$set(
  eval = .soilDB_is_sufficient(),
  collapse = TRUE,
  comment = "#>"
)
```
...
Here we determine the family-level classes for a single taxonomic family and match them to the class descriptions stored in `ST_family_classes`. Here we set `column_metadata=FALSE` to not attempt to parse the family class components into their corresponding NASIS domains (requires soilDB).

```{r}
afamily_classes <- parse_family("fine-loamy, mixed, semiactive, mesic ultic haploxeralfs", column_metadata = FALSE)$classes_split[[1]]
