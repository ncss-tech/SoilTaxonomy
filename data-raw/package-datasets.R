...
ST_criteria_13th <- lapply(ST_criteria_13th, function(x) {
  colnames(x) <- c("content", "chapter", "page", "key", "taxon", "code", "clause", "logic")
  x$logic[is.na(x$logic)] <- "HAVE" # FIX IN SKB
  x
})

usethis::use_data(ST_criteria_13th, overwrite = TRUE)
